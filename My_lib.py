...
def RK4_method2(function2, initValue, h, N):
    ...
    for i in range(N):
        ...
        k4 = h*(Z+(l3/2))
        newPoint = (X+h, Y + (k1+(2*k2)+(2*k3)+k4) /
                    6, Z + (l1+(2*l2)+(2*l3)+l4)/6)
        points.append(newPoint)
    return points

# 27 Boundary value problem
...
def boundaryValueProblem(function, firstBoundaryCondition, secondBoundaryCondition, N, guess, guessVariation, tolerance):
    ...
    if (fb1 > fb0):
        ...
        while(fb1 > fb0):
            ...
            fb1 = RK4_method2(
                function, firstBoundaryCondition+(guess,), h, N)[-1][1]
        GLow = guess
    else:
        GLow = guess
        while(fb1 < fb0):
    ...
    while(abs(fb3 - fb0) > tolerance and abs(fb3) < 100):
        ...
        fb1 = RK4_method2(function, firstBoundaryCondition +
                          (GLow,), h, N)[-1][1]
        fb2 = RK4_method2(function, firstBoundaryCondition +
                          (GHigh,), h, N)[-1][1]
        Gnew = GLow + ((GHigh - GLow)/(fb2-fb1))*(fb0-fb1)
        fb3 = RK4_method2(function, firstBoundaryCondition +
                          (Gnew,), h, N)[-1][1]
